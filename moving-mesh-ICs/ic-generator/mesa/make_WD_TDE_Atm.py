...
if( not args.no_atm) :
    ...
    rArray = np.linalg.norm(atmos_pos, axis=-1)
    boolArray = rArray > r1Max
    ExtPosArray = atmos_pos[boolArray]
    ExtRArray = np.linalg.norm(ExtPosArray, axis=-1)
    ExtVolArray = atmos_vol[boolArray]
    NumExtParticles = int(ExtPosArray.shape[0])
    ExtVelArray = np.zeros([NumExtParticles,3])
    
    ExtMArray = np.exp(np.maximum(-8*ExtRArray/r1Max, np.log(minRhoExt/rhoExt)))*rhoExt*ExtVolArray
