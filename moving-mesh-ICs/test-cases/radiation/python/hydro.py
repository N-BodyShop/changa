...
mp = 1.6e-24
kms = 1e5
FOUR_THIRD_PI_G = 4./3.*math.pi*6.67e-8
xmax = 1.5e10
cspeed = 3e10
fourPi = 4*pi
aRad = 7.56e-15
...
#
    ...
    for i in range(1, xcoord.size-1) :
        ...
        uL[i,:] = grid[i,:] - dudx[:]*dx[i]*0.5*phi
        uR[i,:] = grid[i,:] + dudx[:]*dx[i]*0.5*phi
    uL = bc(xcoord, uL)
    uR = bc(xcoord, uR)
...
def plot( t, x, grid, i=0, summaryData = None, quiet=False):
    ...
    if( writePlots and (PROBLEM == SOUND_WAVE or PROBLEM == LINEAR_WAVE)) : 
        ...
        pl.plot( x, v/cs0, ls="-.", lw=2, label="$v/c_{s,0}$")
    ...
    if( PROBLEM == LINEAR_WAVE or PROBLEM == SOUND_WAVE) :
        ...
        if( not quiet) : 
            print( t, popt)
        summaryData.append([t, popt[0], popt[1]])
