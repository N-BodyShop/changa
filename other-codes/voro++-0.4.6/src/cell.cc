...
/** Calculates the centroids of each face of the Voronoi cell and prints the
 * results to an output stream.
 * \param[out] v the vector to store the results in. */
void voronoicell_base::face_centroids(std::vector<double> &v) {
        double area, weighted_x, weighted_y, weighted_z;
	v.clear();
	int i,j,k,l,m,n;
	double ux,uy,uz,vx,vy,vz,wx,wy,wz;
	for(i=1;i<p;i++) for(j=0;j<nu[i];j++) {
		...
		if(k>=0) {
			...
			l=cycle_up(ed[i][nu[i]+j],k);
			m=ed[k][l];ed[k][l]=-1-m;
			double tri_x, tri_y, tri_z, tri_area;
			while(m!=i) {
				...
				wy=uz*vx-ux*vz;
				wz=ux*vy-uy*vx;
				tri_area=sqrt(wx*wx+wy*wy+wz*wz);
				tri_x=0.5*(pts[3*i] + pts[3*k] + pts[3*m])/3.;
				tri_y=0.5*(pts[3*i+1] + pts[3*k+1] + pts[3*m+1])/3.;
				tri_z=0.5*(pts[3*i+2] + pts[3*k+2] + pts[3*m+2])/3.;
				area+=tri_area;
				weighted_x+=tri_area*tri_x; 
				weighted_y+=tri_area*tri_y; 
				weighted_z+=tri_area*tri_z;
				k=m;l=n;
				m=ed[k][l];ed[k][l]=-1-m;
			}
			weighted_x/=area; weighted_y/=area; weighted_z/=area;
			v.push_back(weighted_x); v.push_back(weighted_y); v.push_back(weighted_z);
		}
	}
	reset_edges();
}
