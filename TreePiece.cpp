...
void TreePiece::receivedSortedParticles(){
  ...
  //CkPrintf("[%d] received particles %d / %d\n", thisIndex, incomingParticlesArrived, dm->particleCounts[myPlace]);
...
void ShuffleShadowArray::process(const ParticleShuffle &shuffle) {
  treeProxy[thisIndex].ckLocal()->acceptSortedParticles(shuffle);
}
