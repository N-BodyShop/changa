...
void TreePiece::acceptSortedParticlesFromOther(ParticleShuffleMsg *shuffleMsg) {
  ...
  if (totalShuffleSize < (myShuffleLocG + 1 + shuffleMsg->n)) {
    totalShuffleSize *= 2;
    if (totalShuffleSize < (myShuffleLocG + 1 + shuffleMsg->n)) {
      totalShuffleSize = 1.5 * (myShuffleLocG +1 + shuffleMsg->n);
    }

    myTmpShuffleParticle.resize(totalShuffleSize);
  }
  ...
  if (totalShuffleSphSize < (myShuffleLocSph + 1 + shuffleMsg->nSPH)) {
    totalShuffleSphSize *= 2;
    if (totalShuffleSphSize < (myShuffleLocSph + 1 + shuffleMsg->nSPH)) {
      totalShuffleSphSize = 1.5 * (myShuffleLocSph + 1 + shuffleMsg->nSPH);
    }

    myTmpShuffleSphParticle.resize(totalShuffleSphSize);
  }
  ...
  if (totalShuffleStarSize < (myShuffleLocStar + 1 + shuffleMsg->nStar)) {
    totalShuffleStarSize *= 2;
    if (totalShuffleStarSize < (myShuffleLocStar + 1 + shuffleMsg->nStar)) {
      totalShuffleStarSize = 1.5 * (myShuffleLocStar + 1 + shuffleMsg->nStar);
    }

    myTmpShuffleStarParticle.resize(totalShuffleStarSize);
  }
...
void TreePiece::mergeAllParticlesAndSaveCentroid() {
  ...
  if (myShuffleLocG > 0) {
    ...
    sort(myTmpShuffleParticle.begin(),
      myTmpShuffleParticle.begin() + myShuffleLocG + 1);
  }
