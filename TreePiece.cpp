...
void ReductionHelper::evaluateBoundaries(bool convertToLoad, SFC::Key* keys, const int n, int skipEvery, const CkCallback& cb){
  if(localTreePieces.presentTreePieces.size() == 0){
    ...
    float *dummy = new float[numBins];
    for(int i = 0; i < numBins; i++) dummy[i] = 0;
    contribute(sizeof(float)*numBins, dummy, CkReduction::sum_float, cb);
    return;
  }
...
void TreePiece::evaluateBoundaries(bool convertToLoad, SFC::Key* keys, const int n, int skipEvery, const CkCallback& cb){
...
#endif
  if (convertToLoad) {
    myLoads = new float[numBins];
    memset(myLoads, 0, numBins * sizeof(float));
  }
  ...
  if (myNumParticles > 0) {    
    ...
    for( ; keyIter != endKeys; ++keyIter) {
      ...
      if (skip != 0) {
        ...
        if (convertToLoad) {
          myLoads[binIter] = treePieceLoadTmp * myCounts[binIter] / myNumParticles;
        }
        ++binIter;
        --skip;
      } else {
  ...
  */
  ...
  if (!convertToLoad) {
    ...
    contribute(numBins * sizeof(float), myLoads, CkReduction::sum_float, cb); 
  }
#endif
}
  ...
  for (unsigned int i=0; i<node->numChildren(); ++i) {
    ...
    if (child->getType() == NonLocal) {
      ...
      if (last < first) {
	      ...
	      streamingProxy[child->remoteIndex].requestRemoteMoments(child->getKey(), thisIndex.data[0], &opts);
...
void TreePiece::mergeNonLocalRequestsDone(){
  ...
  if(root->remoteIndex == thisIndex.data[0]){
    treeBuildComplete();
  }
}
...
void TreePiece::deliverMomentsToClients(const std::map<NodeKey,NonLocalMomentsClientList>::iterator &it){
  ...
  */
  ...
  for(int i = 0; i < clients.length(); i++){
    MERGE_REMOTE_REQUESTS_VERBOSE("[%d] send %llu (%s) moments to %d\n", thisIndex.data[0], node->getKey(), typeString(node->getType()),clients[i].clientTreePiece->getIndex());
...
void TreePiece::walkBucketTree(GenericTreeNode* node, int reqID) {
  ...
  if(!openCriterionBucket(node, reqnode, offset, localIndex)) {
...
#endif
#if COSMO_PRINT > 1
    CkPrintf("[%d] walk bucket %s -> node %s\n",thisIndex.data[0],keyBits(bucketList[reqIDlist]->getKey(),KeyBits).c_str(),keyBits(node->getKey(),KeyBits).c_str());
#endif
#if COSMO_DEBUG > 1
    ...
    for(int i = node->firstParticle; i <= node->lastParticle; ++i) {
      ...
      CkPrintf("[%d] walk bucket %s -> part %016llx\n",thisIndex.data[0],keyBits(reqnode->getKey(),KeyBits).c_str(),node->particlePointer[i-node->firstParticle].key);
#endif
#ifdef COSMO_EVENTS
      double startTimer = CmiWallTimer();
#endif
...
GenericTreeNode* TreePiece::requestNode(int remoteIndex, Tree::NodeKey key, int chunk, int reqID, int awi, void *source, bool isPrefetch) {
  ...
  if(_cache){
    ...
    CkPrintf("[%d] b=%d requesting node %s to %d for %s (additional=%d)\n",thisIndex.data[0],reqID,keyBits(key,KeyBits).c_str(),remoteIndex,keyBits(bucketList[reqID]->getKey(),KeyBits).c_str(),
...
void ReductionHelper::reduceBinLoads(int nBins, float *binLoads, const CkCallback &cb){
  ...
  if (numTreePiecesCheckedIn == 1) {
    ...
    memcpy(&myBinLoads[0], binLoads, nBins*sizeof(float));
  }
  else{
  ...
  if(numTreePiecesCheckedIn == localTreePieces.presentTreePieces.size()){
    //CkPrintf("ReductionHelper %d contributing to PE-level reduction\n", CkMyPe());
    numTreePiecesCheckedIn = 0;
    contribute(sizeof(float)*myBinLoads.size(), &myBinLoads[0], CkReduction::sum_float, cb);
  }
}
