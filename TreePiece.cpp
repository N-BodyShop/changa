...
void TreePiece::recvRightBoundary(SFC::Key key) {
  ...
  // Since this TreePiece doesn't have any particles, forward the right boundary
  // received to the left neighbor
  if (myNumParticles <= 0) {
    int myPlace;
    if (dm == NULL)
      dm = (DataManager*)CkLocalNodeBranch(dataManagerID);
    ...
    if (myPlace != 0) {
      thisProxy[dm->responsibleIndex[myPlace-1]].recvRightBoundary(key);
    }
  }
}
...
void TreePiece::recvLeftBoundary(SFC::Key key) {
  ...
  // Since this TreePiece doesn't have any particles, forward the left boundary
  // received to the right neighbor
  if (myNumParticles <= 0) {
    int myPlace;
    if (dm == NULL)
      dm = (DataManager*)CkLocalNodeBranch(dataManagerID);
    ...
    if (myPlace != (dm->responsibleIndex.size()-1)) { 
      thisProxy[dm->responsibleIndex[myPlace+1]].recvLeftBoundary(key);
    }
  }
}
...
void TreePiece::buildORBTree(GenericTreeNode * node, int level){
  ...
  for (unsigned int i=0; i<node->numChildren(); ++i) {
    ...
    if (child->getType() == NonLocal) {
      ...
      // This assert is not valid with the tree build where we use splitter keys
      //CkAssert(!isShared);
      if (last < first) {
