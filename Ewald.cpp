...
void TreePiece::BucketEwald2D(GenericTreeNode *req, int nReps,double fEwCut)
{
	GravityParticle *p;
#ifdef HEXADECAPOLE
	MOMC mom = momcRoot;
...
#endif
	double Q2;
	double fEwCut2,fInner2,alpha,alpha2,alphan,ka;
	double fPot,ax,ay,az;
	double dx,dy,dz,x,y,z,r2,dir,dir2,a;
	double Q2mirx,Q2miry,Q2mirz,Q2mir,Qta;
	double g0,g1,g2,g3,g4,g5;
	double hdotx,s,c;
	int i,j,n,ix,iy,iz,nEwReps,bInHole,bInHolex;
	int nLoop = 0;
	...
	p = &myParticles[req->firstParticle];
	nEwReps = (int) ceil(fEwCut);
	double Lmax = fmax(fPeriod.x, fPeriod.y);
	fEwCut2 = fEwCut*fEwCut*Lmax*Lmax;
	fInner2 = 1.2e-3*Lmax*Lmax;
	nEwReps = nEwReps > nReps ? nEwReps : nReps;
	alpha = 2.0/sqrt(fPeriod.x*fPeriod.y); // Geometric mean
	alpha2 = alpha*alpha;
	ka = 2.0*alpha/sqrt(M_PI);

        int hReps = (int) ceil(dEwhCut);
        double dArea = fPeriod.x*fPeriod.y;
        
	for(j=0;j<n;++j) {
            if (p[j].rung < activeRung) continue;
#ifdef HEXADECAPOLE
            fPot = momQuad.totalMass*ka;
#else
            fPot = mom.totalMass*ka;
#endif
            ax = 0.0;
            ay = 0.0;
            az = 0.0;
            ...
            for (ix=-nEwReps;ix<=nEwReps;++ix) {
                ...
                x = dx + ix*fPeriod.x;
                for(iy=-nEwReps;iy<=nEwReps;++iy) {
                    bInHole = (bInHolex && iy >= -nReps && iy <= nReps);
                    y = dy + iy*fPeriod.y;
                    z = dz;
                    r2 = x*x + y*y + z*z;
                    if (r2 > fEwCut2 && !bInHole) continue;
            ...
            for (int hx=-hReps;hx<=hReps;++hx) {
                for (int hy=-hReps;hy<=hReps;++hy) {
                    double h2 = hx*hx + hy*hy;
                    if (h2 == 0) continue;
                    if (h2 > dEwhCut*dEwhCut) continue;
                    double kx = 2.0*M_PI/fPeriod.x;
                    double ky = 2.0*M_PI/fPeriod.y;
                    hdotx = kx*dx + ky*dy;
                    double dExpErfcPlus;    /* Big exp and erfc term */
                    double dExpErfcMinus;    /* Big exp and erfc term */
                    double dKabs;
                    dKabs = sqrt(hx*hx*kx*kx + hy*hy*ky*ky);
                    dExpErfcPlus = exp(dKabs*dz)*erfc(0.5*dKabs/alpha + alpha*dz);
                    dExpErfcMinus = exp(-dKabs*dz)*erfc(0.5*dKabs/alpha - alpha*dz);
                    c = cos(hdotx)*momQuad.totalMass;
                    s = sin(hdotx)*momQuad.totalMass;
                    fPot -= (M_PI/(dKabs*dArea))*c*(dExpErfcPlus + dExpErfcMinus);
                    ax += (M_PI/(dKabs*dArea))*hx*kx*s
                        *(dExpErfcPlus + dExpErfcMinus);
                    ay += (M_PI/(dKabs*dArea))*hy*ky*s
                        *(dExpErfcPlus + dExpErfcMinus);
                    az += (M_PI/dArea)*c*(dExpErfcPlus - dExpErfcMinus);
                }
            }
            fPot += momQuad.totalMass*(2.0*sqrt(M_PI)/dArea)
                *(exp(-alpha*alpha*dz*dz)/alpha + sqrt(M_PI)*dz*erf(alpha*dz));
            az -= momQuad.totalMass*(2.0*M_PI/dArea)*erf(alpha*dz);
...
void TreePiece::EwaldInit2D()
{
	...
	momRescaleFmomr(&(root->moments.mom),root->moments.getRadius(),1.0f);
#endif
        // At the moment there is very little common calculations for
        // the h loop
#if 0
	/*
	 ** Now setup stuff for the h-loop.
