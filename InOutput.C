			      ...
			      const int bCool)
{
	...
	treeProxy[thisIndex.data[0]+1].parallelWrite(iPass + 1, cb, filename, dTime, dvFac, duTFac, bCool);
    contribute(cb);
    }
...
void TreePiece::outputIntASCII(OutputIntParams& params, // specifies
  ...
  if(bParaWrite) {
      ...
      if(thisIndex.data[0]!=(int)numTreePieces-1) {
	  pieces[thisIndex.data[0] + 1].outputIntASCII(params, bParaWrite, cb);
	  return;
	  }
      ...
      pieces[0].oneNodeOutIntArr(params, aiOut, myNumParticles, thisIndex.data[0], cb);
      delete [] aiOut;
      }
}
