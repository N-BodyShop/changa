...
echo "# theta a_rms a_max time" > acc_cha_test4.out
...
./ChaNGa -nrep $NREPS -theta $theta test4_changa.param >& DIAG
$TESTDIR/array/subarr test4.000000.acc2 ../periodic/test4.direct.acc > diff.acc
$TESTDIR/array/magvec < diff.acc > magdiff.arr
$TESTDIR/array/magvec < ../periodic/test4.direct.acc > mag.acc
...
echo $theta $RMS $MAX $TIME >> acc_cha_test4.out
