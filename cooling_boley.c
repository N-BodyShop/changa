...
double clThermalEnergy( COOL * cl, double Y_Total, double T ) {
  return Y_Total/(cl->dParam3-1.0)*CL_Eerg_gm_degK*T;

}

double clTemperature( COOL *cl, double Y_Total, double E ) {
  return E*(cl->dParam3-1.0)/(Y_Total*CL_Eerg_gm_degK);
}
...
int clDerivs(double x, const double *y, double *dydx, void *Data) {
  ...
  d->E = y[0];
  d->T = clTemperature( d->cl, d->Y_Total, d->E );
...
void clIntegrateEnergy(COOL *cl, STIFF *sbs, double *E, 
  ...
  EMin = clThermalEnergy( cl, d->Y_Total, cl->Tmin );
  ...
  gsl_odeiv_evolve_reset(sbs->evolver);
  {
    ...
    while (t<tstop) {
      ...
      d->E = *E;
      d->T = clTemperature( cl, d->Y_Total, d->E );
      clDerivs(t, E, &dEdt, d);
      if (fabs(dEdt) > 0) {
		  dtEst = fabs(*E/dEdt);
  ...
  */
  ...
  if (d->T < cl->Tmin ) {
	  d->T = cl->Tmin;
	  *E = clThermalEnergy( cl, d->Y_Total, d->T );
	  }
}
...
void CoolInitEnergyAndParticleData( COOL *cl, COOLPARTICLE *cp, double *E, double dDensity, double dTemp, double dMetals )
{
	...
	*E = clThermalEnergy(cl,cp->Y_Total,dTemp)*cl->diErgPerGmUnit;
	}
...
double CoolEnergyToTemperature( COOL *Cool, COOLPARTICLE *cp, double E ) {
	return clTemperature( Cool, cp->Y_Total, E );
	}
...
void CoolIntegrateEnergyCode(COOL *cl, STIFF *sbs, COOLPARTICLE *cp,
	...
	*ECode = CoolCodeEnergyToErgPerGm( cl, *ECode );
	clIntegrateEnergy(cl, sbs, ECode, CoolCodeWorkToErgPerGmPerSec( cl, PdVCode ), 
