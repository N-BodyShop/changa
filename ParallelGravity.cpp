  ...
  while (currentStep < MAXSUBSTEPS) {
    ...
    if((param.bStarForm || param.bFeedback)
       && param.stfm->isStarFormRung(activeRung)) {
        ...
        treeProxy.sendParticleInfo(CkCallbackResumeThread((void*&)msg_lb), PHASE_FEEDBACK);
        int64_t* part_data = (int64_t *) msg_lb->getData(); 
        bool doLB = ((float)part_data[0]/part_data[1] > 0.0001) ? true : false;
        delete msg_lb;
        if (doLB) {
    ...
    startTime = CkWallTimer();
    CkReductionMsg *msg_lb;
    treeProxy.sendParticleInfo(CkCallbackResumeThread((void*&)msg_lb), activeRung);
    int64_t* part_data = (int64_t *) msg_lb->getData(); 
    bool doLB = ((float)part_data[0]/part_data[1] > 0.0001) ? true : false;
    delete msg_lb;
    if (doLB) {
