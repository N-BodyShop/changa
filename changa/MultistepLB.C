...
void MultistepLB::qsort(int x, int p, int r)
{
  if (p<r) {
  ...
  for(i = 0; i < stats->n_objs; i++){
    ...
    if(map.tpCentroids[i].numActiveParticles == 0){
      ...
      if(stats->objData[map.tpCentroids[i].tag].migratable){
        stats->objData[map.tpCentroids[i].tag].migratable = 0;
#ifdef MCLBMSV
...
#define ceil(a,b) (a%b == 0? a/b : a/b+1)
  ...
  for(i = 0; i < stats->n_objs; i++)
    objCentroids[map.tpCentroids[i].tag] = map.tpCentroids[i].vec;      // now objCentroids[i] contains centroid of object i
